...
async def ifconfig(ctx):
    iaj = requests.get("https://ifconfig.me/all.json")
    ij = iaj.json()
    ip_addr = ij["ip_addr"]
    remote_host = ij["remote_host"]
    port = ij["port"]
    method = ij["method"]
    encoding = ij["encoding"]
    via = ij["via"]
    forwarded = ij["forwarded"]

    ip_addr = "L'indirizzo IP è: " + str(ip_addr) + "\n"
    remote_host = "Il remote host è: " + str(remote_host) + "\n"
    port = "La porta è: " + str(port) + " (solo per questa request)\n"
    method = "Method: " + str(method) + "\n"
    encoding = "Codifica: " + str(encoding) + "\n"
    via = "Via: " + str(via) + "\n"
    forwarded = "Forwarded: " + str(forwarded) + "\n"
    finalembed = ip_addr + remote_host + port + method + encoding + via + forwarded
    embed_ip = discord.Embed(
        title = 'Ifconfig di ' + username,
        description = finalembed,
        color = discord.Color.blue()
    )
